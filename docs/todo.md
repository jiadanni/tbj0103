...
- [x] **L1 — Linux scroll smoothness**: Mitigated jerky wheel scrolling in chat on Linux/WebKitGTK. Three landed changes: (1) `apply_linux_webkit_env()` in `src-tauri/src/lib.rs` sets `WEBKIT_FORCE_COMPOSITING_MODE=1`, `WEBKIT_DISABLE_DMABUF_RENDERER=0`, `WEBKIT_DISABLE_COMPOSITING_MODE=0` at process startup (Linux-only, respects pre-existing env); (2) `src/lib/linuxWheelSmoother.ts` converts GTK line/page-stepped wheel events to pixel deltas at the document level, skipping CodeMirror and `data-no-wheel-smoothing` opt-outs; (3) `increaseViewportBy={{ top: 1200, bottom: 1200 }}` added to the chat Virtuoso in `ChatView.tsx` so off-screen bubbles paint ahead of the viewport instead of arriving as scroll hitches. Diagnosed via a temporary `[wheel]`/`[scroll]`/`[frame]` instrumenter (removed). Remaining hitches are paint cost in WebKitGTK that further code-side changes can't cheaply address — `content-visibility: auto` was tried and made it worse. Long-term lever would be lazy/virtualized code-block syntax highlighting in `ChatMessageBubble`.
