...
class MLXService: ObservableObject {
    ...
    private let baseURL = "http://localhost:11435"
    private let logger = Logger(subsystem: "com.aetherium.app", category: "MLXService")
    ...
    func checkAvailability() async -> Bool {
        ...
        do {
            ...
            logger.warning("MLX availability check failed: \(error.localizedDescription)")
            isAvailable = false
        }
    ...
    func fetchAvailableModels() async -> [ModelConfiguration] {
        ...
        // Minimal placeholder model entry; real implementation should parse remote response
        return [ModelConfiguration(id: UUID(), name: "mlx-default", provider: "mlx", contextWindow: 8192, isLocal: true, displayName: "MLX Default")]
    }
