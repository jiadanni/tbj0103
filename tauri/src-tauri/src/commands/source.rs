...
use crate::models::source::{Source, CreateSourceRequest};
...
pub fn list_sources(state: State<DbState>, workspace_id: String, source_type: Option<String>) -> Result<Vec<Source>, String> {
    ...
    let items = stmt.query_map(params_refs.as_slice(), row_to_source)
        .map_err(|e| e.to_string())?
...
pub fn update_source(
    ...
    conn.execute(
        "UPDATE sources SET
            ...
            updated_at = ?5
         WHERE id = ?6",
        rusqlite::params![title, summary, is_processed.map(|v| v as i32), folder, now, id],
    ).map_err(|e| e.to_string())?;
    Ok(())
}
