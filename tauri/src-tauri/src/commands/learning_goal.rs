...
use crate::models::learning_goal::{LearningGoal, CreateLearningGoalRequest, UpdateLearningGoalRequest};
...
pub fn create_learning_goal(state: State<DbState>, req: CreateLearningGoalRequest) -> Result<LearningGoal, String> {
    ...
    if let Some(prereqs) = req.prerequisite_ids { g.prerequisite_ids = prereqs; }
...
pub fn update_learning_goal(state: State<DbState>, req: UpdateLearningGoalRequest) -> Result<(), String> {
    ...
    conn.execute(
        "UPDATE learning_goals SET
            ...
            updated_at = ?7
         WHERE id = ?8",
        rusqlite::params![req.title, req.goal_description, req.progress,
                          req.is_completed.map(|b| b as i32), req.due_date, prereq_json, now, req.id],
    ).map_err(|e| e.to_string())?;
    Ok(())
}
