//! knowledge_analysis — AI-powered knowledge graph analysis business logic.
//!
//! Extracted from `commands::ai_knowledge` so the Tauri command layer stays
//! thin (validate input, acquire the pool, delegate, return the result).
//! Owns workspace item gathering/chunking, the per-chunk LLM analysis
//! worker, the semantic dedup pass, and the chunked-analysis orchestrator.
...
pub(crate) fn gather_workspace_items(
...
pub async fn run_semantic_dedup_pass(
    pool: &Pool<SqliteConnectionManager>,
    workspace_id: &str,
    hierarchy_level: &str,
    model: &str,
...
fn auto_categorize_orphans(
    ...
    _workspace_id: &str,
    _name_to_id: &mut HashMap<String, String>,
) -> Result<usize, String> {
    // Leave truly orphaned concepts as-is rather than fabricating an
    // "Uncategorized" chapter scaffold in the roadmap.
    Ok(0)
}

// --------------- Chunked orchestrator ---------------

pub async fn analyze_workspace_chunked_impl(
    app: &AppHandle,
    pool: &Pool<SqliteConnectionManager>,
