...
export default function ComposerSuggestionRows({
  ...
  const renderSuggestion = (suggestion: ComposerSuggestion) => {
    ...
    return (
      <Tooltip key={suggestion.id} delay={600} position="bottom" className={isImmediate ? "" : "!whitespace-normal text-center"} content={isImmediate ? "Send immediately" : <span>Add to composer<br />Ctrl+click to send</span>}>
        <button
          key={suggestion.id}
          type="button"
          disabled={isDisabled}
