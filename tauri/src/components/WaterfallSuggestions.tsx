...
export function WaterfallSuggestions({ suggestions, onSelect }: WaterfallSuggestionsProps) {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none select-none grid grid-cols-[minmax(0,1fr)_minmax(0,28rem)_minmax(0,1fr)] gap-8 px-4 md:px-8 opacity-55 [mask-image:linear-gradient(to_bottom,transparent_0%,black_18%,black_76%,transparent_100%)]">
      {columns.map((col, i) => (
        <div
          key={i}
