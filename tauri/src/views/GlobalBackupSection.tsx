...
export default function GlobalBackupSection() {
  ...
  async function handleImported(result: SelectiveImportResult) {
    ...
    try {
      const nextWorkspaces = await api.workspace.list();
      setWorkspaces(nextWorkspaces);
      const firstId = result.workspace_ids[0];
      if (firstId) {
        const projects = await api.folder.list(firstId);
        setFoldersForWorkspace(firstId, projects);
        setActiveWorkspaceId(firstId);
    ...
    setSuccessDialog({
      ...
      description: summary
        ? `Imported ${result.rows_imported} items across ${result.workspace_ids.length} workspace${result.workspace_ids.length === 1 ? "" : "s"}: ${summary}.`
        ...
        <div className="mb-4 flex items-center gap-3 rounded-xl border border-[var(--accent-color)]/20 bg-[var(--accent-color)]/5 px-4 py-3 text-sm text-[var(--text-secondary)] shadow-sm">
          ...
          <div className="flex-1">
            <span className="font-medium text-[var(--text-primary)]">
              {creating ? "Creating global backup..." : previewing ? "Reading backup..." : "Restoring global backup..."}
            </span>
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-3 text-xs text-[var(--text-muted)]">
                Restore brings back every workspace in the file. Use selective import to choose
                which workspaces and which data come back.
              </p>
            </div>
